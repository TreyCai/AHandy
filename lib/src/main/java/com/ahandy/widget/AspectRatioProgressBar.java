...
/**
 * {@link android.widget.ProgressBar} which forces itself to be laid out according the aspect ratio.
 * Never use this with <b>Widget.ProgressBar.Horizontal</b> styles.
 */
public class AspectRatioProgressBar extends ProgressBar {
    ...
    public AspectRatioProgressBar(Context context) {
        super(context);
    }

    public AspectRatioProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public AspectRatioProgressBar(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.AspectRatioProgressBar);
        mAspectRatio = Math.abs(a.getFloat(R.styleable.AspectRatioProgressBar_aspectRatio, 1));
        a.recycle();
    }
